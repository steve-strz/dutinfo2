...
<html lang="fr">
  ...
  <body>
    ...
    <form action="exo15.php" method="GET" class="question">
        ...
        <?php
          if(!empty($_GET['typeVin'])){
            if(preg_match("([^A-Za-z \s \-])", $_GET['surname'])) echo "Le nom peut contenir les caractères suivant : A-Z, a-z, espace, tiret.";
              ...
              echo "Bonjour " .$_GET['civilite']. " " .$_GET['name']. " " .$_GET['surname']. ", <br/>";
              echo "Nous vous remercions d'avoir commandé : <br/>";
              echo "<ol>";
              foreach ($_GET['typeVin'] as $key => $value) {
                echo "<li>".$value."</li>";
              }
              echo "</ol>";
              echo "Un mail de confirmation vous a été envoyé à l'adresse : " .$_GET['email'];
            }
          }
